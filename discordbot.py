...
def get_gs():
    ...
    #OAuth用クライアントIDの作成でダウンロードしたjsonファイル
    json_file = conf['kyoto-rhapsody-09c16eb705d4.json']
    credentials = ServiceAccountCredentials.from_json_keyfile_name(json_file, scopes=scopes)
    http_auth = credentials.authorize(Http())

    # スプレッドシート用クライアントの準備
    #これはスプレッドシートのURLのうちhttps://docs.google.com/spreadsheets/d/以下の部分です
    doc_id = conf['1Jj54SjthyAQKOWIvjoKhRTeMVWoQPOAjLZcRW5xCJ_Q/edit#gid=0']
    gs = gspread.authorize(credentials)
    #読み書きするgoogle spreadsheet
...
def get_charactor(sheet_name):
    ...
    charactor = {}
    #技能名カラム
    cell_keys = worksheet.col_values(3)
    #合計値カラム
    cell_values = worksheet.col_values(4)
    cell_dice = worksheet.col_values(12)
